...
export const ThemeSettingsSection: React.FC<ThemeSettingsSectionProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'background' | 'fonts' | 'glass'>('glass');
  ...
  // Glass Surface Helpers & State
  const isLiquid = themeConfig.liquidGlassMode === 'crystal' || (!['blur', 'opaque', 'off'].includes(themeConfig.liquidGlassMode));
  const isBlur = themeConfig.liquidGlassMode === 'blur';
  const isOpaque = themeConfig.liquidGlassMode === 'opaque' || themeConfig.liquidGlassMode === 'off';

  const handleSelectGlassMode = (mode: 'crystal' | 'blur' | 'opaque') => {
    const updated: UiThemeConfig = {
      ...themeConfig,
    ...
    saveThemeConfig(updated);
    if (showToast) {
      if (mode === 'crystal') showToast('💧 Liquid Glass Active');
      else if (mode === 'blur') showToast('🌫️ Standard Blur Active');
      else showToast('⬛ Plain Opaque Active');
    }
  };

  const handleGlassBlurChange = (val: number) => {
    const updated: UiThemeConfig = {
      ...themeConfig,
      glassBlur: val,
    };
  ...
  const handleRefractionHeightChange = (val: number) => {
    const updated: UiThemeConfig = {
      ...themeConfig,
      refractionHeight: val,
    };
    onThemeChanged(updated);
    saveThemeConfig(updated);
  };
        ...
        <div className="rounded-3xl glass-card-themed p-4 space-y-4">
          ...
          <div className="space-y-3 w-full">
            ...
            <div className="space-y-2.5">
              <button
                type="button"
                onClick={() => handleSelectGlassMode('crystal')}
                className={`w-full py-3 px-4 sm:px-5 rounded-full text-xs font-semibold transition-all flex items-center justify-between cursor-pointer min-h-[46px] ${
                  isLiquid
                    ? 'bg-white text-neutral-950 font-bold shadow-[0_2px_14px_rgba(255,255,255,0.25)] ring-1 ring-white'
                    : 'bg-white/[0.06] border border-white/10 text-neutral-300 hover:bg-white/10 hover:text-white'
                }`}
              >
                <div className="flex items-center gap-2.5">
              ...
              {/* Popdown between Liquid Glass and Standard Blur */}
              <AnimatePresence>
                {isLiquid && (
                  <motion.div
                    key="liquid-sliders"
                    ...
                    className="space-y-3.5 px-3 py-1.5 overflow-hidden"
                  >
                    {/* Blur Slider */}
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold text-white">Blur</span>
                        <span className="text-xs font-medium text-neutral-300">
                          {themeConfig.glassBlur ?? 1}px
                        </span>
                      </div>
                      <input
                        type="range"
            ...
            {/* Pill 2: Standard Blur */}
            <div className="space-y-2.5">
              <button
                type="button"
                onClick={() => handleSelectGlassMode('blur')}
                className={`w-full py-3 px-4 sm:px-5 rounded-full text-xs font-semibold transition-all flex items-center justify-between cursor-pointer min-h-[46px] ${
                  isBlur
                    ? 'bg-white text-neutral-950 font-bold shadow-[0_2px_14px_rgba(255,255,255,0.25)] ring-1 ring-white'
                    : 'bg-white/[0.06] border border-white/10 text-neutral-300 hover:bg-white/10 hover:text-white'
              ...
              {/* Popdown between Standard Blur and Plain Opaque */}
              <AnimatePresence>
                {isBlur && (
                  <motion.div
                    key="blur-sliders"
                    ...
                    exit={{ opacity: 0, height: 0 }}
                    transition={{ duration: 0.24, ease: [0.25, 0.1, 0.25, 1] }}
                    className="space-y-1.5 px-3 py-1.5 overflow-hidden"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-white">Blur</span>
                      <span className="text-xs font-medium text-neutral-300">
                        {themeConfig.glassBlur ?? 14}px
                      </span>
                    </div>
                    <input
                      type="range"
                      ...
                      max="32"
                      step="1"
                      value={themeConfig.glassBlur ?? 14}
                      onChange={(e) => handleGlassBlurChange(Number(e.target.value))}
                      className="w-full accent-white h-1.5 bg-white/10 rounded-lg cursor-pointer"
                    />
                  </motion.div>
            ...
            <div className="space-y-2.5">
              <button
                type="button"
                onClick={() => handleSelectGlassMode('opaque')}
                className={`w-full py-3 px-4 sm:px-5 rounded-full text-xs font-semibold transition-all flex items-center justify-between cursor-pointer min-h-[46px] ${
                  isOpaque
                    ? 'bg-white text-neutral-950 font-bold shadow-[0_2px_14px_rgba(255,255,255,0.25)] ring-1 ring-white'
                    : 'bg-white/[0.06] border border-white/10 text-neutral-300 hover:bg-white/10 hover:text-white'
